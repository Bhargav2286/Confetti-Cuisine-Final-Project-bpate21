...
module.exports = {
    index: (req, res, next) => {
      Vacations.find()
        ...
        .catch(error => {
          console.log(`Error fetching vacationss: ${error.message}`);
          next(error);
        });
    },
    indexView: (req, res) => {
      res.render("vacations/va_index");
    },
  
    new: (req, res) => {
      res.render("vacations/va_new");
    },
    ...
    if (currentUser) {
      let mappedVacations = res.locals.vacations.map(vacations => {
        let userJoined = currentUser.vacations.some(userVacations => {
          return userVacations.equals(course._id);
        });
  ...
  join: (req, res, next) => {
    ...
    if (currentUser) {
      ...
      next(new Error("User must be log in."));
    }
  }
};